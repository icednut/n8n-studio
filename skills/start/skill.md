...
if [ -n "$EXISTING_PANE" ]; then
  ...
  echo "기존 pane 재사용: $PANE_NAME ($PANE_ID)"
else
  # 3b. 새 pane 생성 → 즉시 이름 부여 → claude 시작
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  [$PANE_NAME] pane (ID: $PANE_ID)에서 실행 중"
echo "  포커스 전환: Ctrl+b → 방향키 또는 Ctrl+b o"
echo "  완료 시 Team Lead에게 자동 보고됩니다."
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
```
...
## 3단계: workflow-planner 디스패치 (Pane: n8n-planner)
...
1. `[docs_path]/spec.md`를 읽어 사용자에게 보여준다
2. 승인 또는 수정 요청을 받는다
...
## 4단계: workflow-designer 디스패치 (Pane: n8n-designer)
...
2. Team Lead에게 보고:
   tmux send-keys -t [TEAM_LEAD_PANE_ID] "✅ [n8n-designer] 완료. design.md 및 테스트 시나리오 작성 완료. 다음 단계를 진행해주세요." Enter
```

**4단계 완료 후 사용자 검토 (ralf_mode=false인 경우에만):**

1. `[docs_path]/design.md`를 읽어 사용자에게 보여준다
2. 승인 또는 수정 요청을 받는다
...
## 5·6단계: workflow-developer 디스패치 (Pane: n8n-developer)
...
## 7단계: workflow-verifier 디스패치 (Pane: n8n-verifier, RALF 루프)
...
## 8단계: workflow-finisher 디스패치 (Pane: n8n-finisher)
...
- **2단계**: 워크플로우 ID 질문
- **3단계 완료 후** (ralf_mode=false): spec.md 검토 및 승인 대기
- **4단계 완료 후** (ralf_mode=false): design.md 검토 및 승인 대기
