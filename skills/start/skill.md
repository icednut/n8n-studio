...
```
docs_path:    docs/[프로젝트명]/yyyyMMdd-기능요약/
branch:       feature/작업요약
...
## 0단계: 프로젝트 컨텍스트 파악 (Team Lead 직접 처리 — 1단계 이전 필수 실행)

워크플로우 파악 및 개발 전에 반드시 프로젝트 배경 정보를 읽는다.

### 프로젝트명 탐색

```bash
# docs/ 하위의 프로젝트 디렉토리 탐색
ls docs/ 2>/dev/null || echo "(docs/ 없음)"

# workflow/ 하위의 프로젝트 디렉토리 탐색
ls workflow/ 2>/dev/null || echo "(workflow/ 없음)"
```

- `docs/` 하위 디렉토리 목록과 `workflow/` 하위 디렉토리 목록을 확인하여 프로젝트명 후보를 파악한다
- 프로젝트가 하나뿐이면 자동으로 선택한다
- 여러 프로젝트가 존재하면 사용자에게 어느 프로젝트와 관련된 작업인지 질문한다 (번호 목록으로 제시)

### 프로젝트 문서 읽기

프로젝트명이 확정되면 아래 두 파일을 **반드시 읽는다**:

1. `docs/[프로젝트명]/OVERVIEW.md` — 프로젝트 개요, 목적, 주요 구성 요소
2. `workflow/[프로젝트명]/README.md` — 워크플로우 목록, 역할 분담, 데이터 구조

파일이 존재하지 않는 경우:
- `OVERVIEW.md` 없음: 사용자에게 알리고 계속 진행
- `README.md` 없음: 사용자에게 알리고 계속 진행

### 파악한 정보 유지

읽은 내용을 아래 형태로 내부 상태에 보관하고, 이후 모든 단계의 태스크 파일에 `[프로젝트 컨텍스트]` 섹션으로 포함한다:

```
project_name:    [프로젝트명]
project_overview: [OVERVIEW.md 핵심 요약 2-3줄]
workflow_readme:  [README.md 핵심 요약 2-3줄]
```
...
3. write-spec 스킬로 docs/[프로젝트명]/yyyyMMdd-기능요약/01-spec.md를 작성하세요
...
1. 결과 파일 기록:
   echo "docs_path: docs/[프로젝트명]/[생성한 폴더명]/" > /tmp/n8n-studio/planner_output.md
   echo "branch: feature/[브랜치명]" >> /tmp/n8n-studio/planner_output.md

2. Team Lead에게 보고:
   tmux send-keys -t [TEAM_LEAD_PANE_ID] "✅ [n8n-planner] 완료. docs_path=docs/[프로젝트명]/[폴더명]/, branch=feature/[브랜치명]. 다음 단계를 진행해주세요." Enter
```
...
완료 보고를 받으면 다음을 순서대로 수행한다:

1. `/tmp/n8n-studio/finisher_output.md`에서 PR URL을 읽어 사용자에게 최종 보고한다.

2. **서브에이전트 pane 정리** — team-lead pane만 남기고 나머지 모두 종료한다:
   ```bash
   TEAM_LEAD_PANE=$(cat /tmp/n8n-studio/team_lead_pane.txt)
   TEAM_LEAD_WINDOW=$(cat /tmp/n8n-studio/team_lead_window.txt)

   # team-lead pane을 제외한 동일 윈도우 내 모든 pane 종료
   tmux list-panes -t "$TEAM_LEAD_WINDOW" -F "#{pane_id}" 2>/dev/null | while read PANE_ID; do
     if [ "$PANE_ID" != "$TEAM_LEAD_PANE" ]; then
