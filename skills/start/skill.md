...
전체 8단계 개발 사이클의 오케스트레이터. 각 단계를 **독립 Tmux 창에서 실행되는 새 Claude 인스턴스로 디스패치**하여 컨텍스트를 격리한다.

## 오케스트레이션 원칙

**오케스트레이터(이 스킬)는 최소 상태만 유지한다:**
- 각 단계는 새 Tmux 창에서 독립 실행 → 이전 단계 컨텍스트를 상속받지 않음
- 에이전트 간 정보 전달은 **파일을 통해서만** 이루어짐
- 에이전트는 완료 후 결과를 표준 출력으로 반환 → 오케스트레이터가 파일에서 읽음
- 오케스트레이터는 아래 5가지만 추적:
...
# 4. Tmux 창 생성 및 Claude 실행
tmux new-window -n "$WINDOW_NAME" 2>/dev/null || tmux rename-window -t "$WINDOW_NAME" "$WINDOW_NAME" 2>/dev/null || true
tmux send-keys -t "$WINDOW_NAME" "cd '$WORK_DIR' && claude -p \"\$(cat '$TASK_FILE')\" > '$OUTPUT_FILE' 2>&1; echo 'STAGE_DONE' >> '$STATUS_FILE'" Enter

# 5. 완료 대기 (최대 30분, 10초 간격 폴링)
echo "[$STAGE_NAME] Tmux 창 '$WINDOW_NAME'에서 실행 중... (완료 대기 중)"
TIMEOUT=1800
ELAPSED=0
...
> **참고:** 출력에서 `docs_path:`, `branch:`, `workflow_ids:`, `all_passed:` 등의 키를 파싱하여 오케스트레이터 상태를 업데이트한다.
...
## 2단계: 정보 수집 및 요구사항 명확화 (직접 처리 — 사용자 인터랙션)
...
사용자 요청을 분석하여 불명확하거나 구체적이지 않은 부분을 식별한다.
불명확한 요구사항이 있으면 **구체적인 질문을 목록으로 정리하여** 사용자에게 한 번에 물어본다.

명확화가 필요한 항목의 예:
- 데이터 입출력 형식이 불명확한 경우
- 에러 처리 방식이 명시되지 않은 경우
- 외부 시스템 연동 방식이 모호한 경우
- 트리거 조건이 구체적이지 않은 경우

사용자의 답변을 수집하고, 필요하다면 추가 질문을 계속한다.
**모든 핵심 요구사항이 명확해졌다고 판단될 때만 다음 단계로 진행한다.**

완료 후, 명확화된 전체 요구사항을 내부적으로 정리한다.

---

## 3단계: workflow-planner Tmux 에이전트 디스패치

Tmux 디스패치 패턴으로 아래 태스크를 `planner` 창에서 실행한다:
...
- 사용자 요청 (명확화 완료): [2단계에서 구체화된 전체 요구사항]
- 진입 유형: [초기 진입 / 재진입 (실패 시나리오: ...) / 수정 재진입 (수정 지시: ...)]

[작업]
1. analyze-workflow 스킬로 워크플로우를 분석하세요
2. write-spec 스킬로 docs/yyyyMMdd-기능요약/spec.md를 작성하세요
   (재진입/수정 재진입의 경우 기존 spec.md를 기반으로 수정하세요)
3. git checkout -b feature/작업요약 으로 브랜치를 생성하세요
   (이미 브랜치가 있으면 생략하세요)

완료 시 반드시 아래 형식으로 출력하세요:
docs_path: docs/[생성한 폴더명]/
branch: feature/[브랜치명]
```

완료 후, 출력에서 `docs_path`와 `branch`를 파싱하여 오케스트레이터 상태에 저장한다.

**3단계 완료 후 사용자 검토 (ralf_mode=false인 경우에만):**

1. `[docs_path]/spec.md` 파일을 읽어 내용을 사용자에게 보여준다
2. 사용자에게 아래와 같이 묻는다:
   ```
   spec.md 검토를 요청합니다.
   ...
   없으시면 "승인"이라고 입력해 주세요.
   ```
3. 사용자가 "승인" (또는 동의하는 표현)하면 4단계로 진행한다
4. 사용자가 수정을 요청하면:
   - 수정 지시 내용을 파악하고
   - 3단계를 **수정 재진입** 모드로 재실행한다 (수정 지시 포함)
...
Tmux 디스패치 패턴으로 아래 태스크를 `designer` 창에서 실행한다:
...
1. `[docs_path]/design.md` 파일을 읽어 내용을 사용자에게 보여준다
2. 사용자에게 아래와 같이 묻는다:
   ```
   design.md 검토를 요청합니다.
   수정이 필요한 부분이 있으면 말씀해 주세요.
   없으시면 "승인"이라고 입력해 주세요.
   ```
3. 사용자가 "승인" (또는 동의하는 표현)하면 5단계로 진행한다
4. 사용자가 수정을 요청하면:
   - 수정 지시 내용을 파악하고
   - 4단계를 **수정 재진입** 모드로 재실행한다 (수정 지시 포함)
...
Tmux 디스패치 패턴으로 아래 태스크를 `developer` 창에서 실행한다:
...
완료 후, 출력에서 `workflow_ids`를 파싱하여 오케스트레이터 상태에 저장한다.
...
Tmux 디스패치 패턴으로 아래 태스크를 `verifier` 창에서 실행한다:
...
- `all_passed: false` AND `ralf_cycle < 3`:
  1. 실패 목록을 사용자에게 표시
  2. "재시도합니다 (사이클 [N]/3)" 알림
  3. `ralf_cycle += 1`, **`ralf_mode = true`** 로 설정 (3단계/4단계 사용자 승인 생략)
  4. 3단계로 돌아가 workflow-planner를 **재진입** 모드로 디스패치 (실패 시나리오 전달)
  5. 4단계 → 5단계 → 7단계 반복
- `all_passed: false` AND `ralf_cycle == 3`:
  `ralf_mode = false`로 초기화 후 아래 형식으로 보고하고 사용자 지침 대기:
  ```
  ## 검증 실패 보고 (사이클 3/3)
  ### 실패한 시나리오
  [failure_summary 내용]
...
Tmux 디스패치 패턴으로 아래 태스크를 `finisher` 창에서 실행한다:
