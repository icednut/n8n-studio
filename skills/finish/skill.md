...
`docs/[프로젝트명]/yyyyMMdd-기능요약/04-result.md`에 아래 내용을 작성한다:
...
### 2.5. 워크플로우 Summary 갱신

각 워크플로우에 대해 `summarize-workflow` 스킬을 실행하여 `workflow/[프로젝트명]/summary/[workflow_id].json`을 최신 노드 구조로 갱신한다.

### 2.7. triggerDataFormat 갱신

`summarize-workflow` 직후, 각 워크플로우에 대해 아래를 수행한다:

1. `n8n_get_workflow`로 첫 번째 노드 타입 확인
2. 첫 번째 노드가 **Webhook**이면:
   - `n8n_executions`로 가장 최근 성공 실행(Happy Path) 조회
   - Webhook 노드 Output 데이터를 **가공 없이 그대로** 추출
   - `workflow/[프로젝트명]/summary/[workflow_id].json`의 `triggerDataFormat` 업데이트
3. Webhook이 아니거나 성공 실행이 없으면 건너뛴다
