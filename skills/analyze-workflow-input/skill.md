...
`body`, `headers`, `params`, `query` 구조를 **가공 없이 그대로** 샘플 데이터로 사용한다.

**절대 금지 사항:**
- 문자열 값을 이스케이프 처리하지 않는다 (`\"` → `"` 변환 등 금지)
- JSON 문자열로 직렬화된 값(예: Slack의 `payload` 필드)을 파싱하거나 변환하지 않는다
- 어떠한 형태로도 데이터를 재가공하지 않는다
- MCP 도구가 반환한 실행 데이터를 있는 그대로 `triggerDataFormat`에 할당한다

예시 (Slack slash command):
```json
{
  "body": {
    "user_id": "U123456",
    "command": "/task",
    "text": "add buy milk",
    "payload": "{\"type\":\"block_actions\",\"actions\":[{\"action_id\":\"btn_complete\",\"value\":\"task_001\"}]}"
  },
  "headers": {
...
위 예시에서 `payload` 값이 JSON 문자열로 되어 있더라도 파싱하지 않고 문자열 그대로 저장한다.
중첩 JSON 객체도 마찬가지로 원본 구조를 그대로 유지한다.
