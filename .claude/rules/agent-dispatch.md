...
## 규칙 2-2: 서브에이전트는 사용자와 직접 인터랙션하지 않는다

**모든 서브에이전트는 작업 중 사용자에게 무언가를 물어봐야 하는 상황이 생기더라도 절대 직접 질문하거나 응답을 기다리지 않는다.**

### 처리 방식

서브에이전트가 사용자 입력이 필요한 상황이 생기면 반드시 아래 절차를 따른다:

1. **작업을 그 시점에서 중단한다.**
2. **Team Lead에게 tmux로 질문 내용을 보고한다:**

```bash
tmux send-keys -t [TEAM_LEAD_PANE_ID] "⚠️ [에이전트명] 사용자 확인 필요. 질문: [질문 내용]. 답변을 전달해주세요." Enter
```

3. **Team Lead가 사용자와 인터랙션하여 답변을 얻는다.**
4. **Team Lead가 해당 서브에이전트 pane에 답변을 전달한다:**

```bash
tmux send-keys -t [SUB_AGENT_PANE_ID] "사용자 답변: [답변 내용]. 이어서 작업을 계속해주세요." Enter
```

5. **서브에이전트는 전달받은 답변을 바탕으로 작업을 재개한다.**

### 위반 예시 (금지)

- 서브에이전트가 자신의 pane에서 "어떻게 할까요?" 라고 출력하고 사용자 입력을 기다리는 행위
- 서브에이전트가 선택지를 나열하고 사용자가 직접 번호를 입력하길 기다리는 행위
- 서브에이전트가 판단 불가 상황에서 작업을 멈추고 침묵하는 행위

### 원칙

사용자와의 모든 인터랙션 창구는 **Team Lead 단 하나**다.
서브에이전트는 독립적으로 판단할 수 없는 상황을 만나면 즉시 Team Lead에게 위임한다.
