...
model: haiku
color: cyan
tools: ["Read", "Write", "Bash", "Glob", "Grep", "mcp__n8n-mcp__n8n_list_workflows", "mcp__n8n-mcp__n8n_executions"]
---
...
2. **Analyze workflows**: Use `analyze-workflow` skill
   - Read summary JSON files under `workflow/[프로젝트명]/summary/` in the current project directory
   - Match the target workflow by workflow_id or workflow_name from the summary files
   - Extract node structure (node_id, node_name, node_summary) from matched summary
   - If Execution ID provided: call `n8n_executions` to analyze logs
   - For new workflow development: scan all summary JSON files to find similar workflows and list them as references
   - If no summary files exist, ask the user to run `n8n-studio:summarize-workflow` first, or fall back to `n8n_list_workflows` to help identify the target workflow
