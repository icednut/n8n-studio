# n8n-studio Agent Dispatch Rules

## 적용 범위

이 규칙은 n8n-studio:start 스킬이 실행 중인 **Team Lead 세션**에 적용된다.
...
Team Lead는 완료 여부를 확인하기 위해 파일을 폴링하거나 상태를 능동적으로 조회하지 않는다.
서브에이전트가 작업 완료 시 반드시 아래 방식으로 Team Lead에게 보고한다:
...
검증 실패 시 RALF 재진입은 반드시 아래 순서대로 각 전담 pane에 태스크를 재전송한다:

```
n8n-planner (재진입) → n8n-designer (재진입) → n8n-developer (재진입) → n8n-verifier (재진입)
```

어떤 단계도 건너뛰거나 Team Lead 세션 또는 n8n-verifier 세션에서 직접 처리하지 않는다.
...
## 규칙 6: Pane 생성 즉시 이름을 부여하고 tiled 레이아웃을 적용한다

```bash
PANE_ID=$(tmux split-window -h -d -t "$TEAM_LEAD_WINDOW" -P -F "#{pane_id}")
tmux select-pane -t "$PANE_ID" -T "$PANE_NAME"   # 즉시 이름 부여
tmux select-layout -t "$TEAM_LEAD_WINDOW" tiled   # 타일 레이아웃 적용
```
